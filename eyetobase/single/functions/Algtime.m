function [Rcf,tcf,pcf,Rit,tit,pit,timecf,timeit,resultList,k_num] = Algtime(Ri,ti,ppi)
tic;
k_num=1;
step=100;
iter_max=100;
...
timecf=toc;
resultList=zeros(3,6,30); % max is set to 30;
resultList(:,:,k_num)=[Rcf tcf pcf [rnti0;0;0]];
k_num=k_num+1;
resultList(:,:,k_num)=[Rcf tcf pcf [timecf;0;0]];
Rk=Rcf;
...
while (step_th<step && k_num<iter_max)
    ...
    resultList(:,:,k_num)=[Rit tit pit [toc;0;0]];
end
